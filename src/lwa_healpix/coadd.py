...
def temporal_std_healpix(
    ...
    *nested* is ``False``, NESTED when ``True``.
    ...
    coord_frame : str, optional
        Output coordinate frame passed to
        :func:`reproject.reproject_to_healpix` (e.g. ``\"galactic\"``,
        ``\"icrs\"``, ``\"g\"``, ``\"c\"``).
...
def coadd_fits(
    ...
    * **target_header** — reproject onto a 2-D image grid described by
    ...
    coord_frame : str, optional
        Coordinate frame for the HEALPix output, passed to
        :func:`reproject.reproject_to_healpix` (e.g. ``\"galactic\"``,
        ``\"icrs\"``, ``\"g\"``, ``\"c\"``).  Used when *nside* is given.
        Default is ``\"galactic\"``.
    nested : bool, optional
